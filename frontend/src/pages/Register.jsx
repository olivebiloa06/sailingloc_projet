...
function OwnerDocumentsStep({ onComplete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setGlobalError(err.response?.data?.message || "Erreur lors de l'envoi des documents. Réessaie.");
    } finally {
      setUploading(false);
    }
  };
...
export default function Register() {
  ...
  const initialRole = searchParams.get("role") === "proprietaire" ? "proprietaire" : "locataire";

  const [form, setForm] = useState({ nom: "", prenom: "", email: "", motDePasse: "", confirmation: "", role: initialRole });
  const [errors, setErrors] = useState({});
  const [serverError, setServerError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [showDocStep, setShowDocStep] = useState(false);

  // SEO — doit être avant tout return conditionnel
  ...
  const validate = () => {
    ...
    if (form.confirmation !== form.motDePasse) next.confirmation = "Les mots de passe ne correspondent pas.";
    setErrors(next);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await register({ nom: form.nom, prenom: form.prenom, email: form.email, motDePasse: form.motDePasse, role: form.role });
      if (form.role === "proprietaire") setShowDocStep(true);
