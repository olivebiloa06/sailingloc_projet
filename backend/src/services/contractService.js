...
exports.generateContractPdf = ({ booking, boat, renter, owner }) => {
  return new Promise((resolve, reject) => {
    ...
    // =========================
    // FILIGRANE DIAGONAL SailingLoc (fond)
    // =========================
    doc.save();
    doc.opacity(0.04);
    ...
    doc.fillColor("#FFFFFF").fontSize(26).font("Helvetica-Bold")
      .text("SailingLoc", 50, 32);
    doc.fillColor(SABLE).fontSize(10).font("Helvetica")
      .text("Location de bateaux entre particuliers", 50, 62);
    doc.fillColor("white").opacity(0.7).fontSize(9)
      .text("Agence Pandawan © 2026 — sailingloc.fr", 50, 82);
    doc.opacity(1);

    // Titre du document (droite du header)
    doc.fillColor("#FFFFFF").fontSize(14).font("Helvetica-Bold")
      .text("CONTRAT DE LOCATION", W - 260, 36, { width: 210, align: "right" });
    doc.fillColor(SABLE).fontSize(10).font("Helvetica")
      .text(`Réservation n°${booking.id}`, W - 260, 58, { width: 210, align: "right" });
    doc.fillColor("white").opacity(0.7).fontSize(9)
      .text(`Émis le ${formatDate(new Date())}`, W - 260, 78, { width: 210, align: "right" });
    doc.opacity(1);
    ...
    doc.fillColor(NAVY).fontSize(12).font("Helvetica-Bold")
      .text("PARTIES AU CONTRAT", 56, y + 12);

    // Propriétaire
    doc.fillColor(MID_GRAY).fontSize(9).font("Helvetica")
      .text("PROPRIÉTAIRE", 56, y + 32);
    doc.fillColor(NAVY).fontSize(10).font("Helvetica-Bold")
      .text(`${owner.prenom} ${owner.nom}`, 56, y + 44);
    doc.fillColor(MID_GRAY).fontSize(9).font("Helvetica")
      .text(owner.email, 56, y + 57);
    ...
    // Locataire
    doc.fillColor(MID_GRAY).fontSize(9).font("Helvetica")
      .text("LOCATAIRE", W / 2 + 20, y + 32);
    doc.fillColor(NAVY).fontSize(10).font("Helvetica-Bold")
      .text(`${renter.prenom} ${renter.nom}`, W / 2 + 20, y + 44);
    doc.fillColor(MID_GRAY).fontSize(9).font("Helvetica")
      .text(renter.email, W / 2 + 20, y + 57);
    ...
    const boatFields = [
      ...
      ["Capacité", `${boat.capacite} personne${boat.capacite > 1 ? "s" : ""} max.`],
    ];
    ...
    doc.fillColor("#FFFFFF").fontSize(10).font("Helvetica-Bold")
      .text(`TOTAL PAYÉ : ${booking.montantTotal} €`, 60, y + 101, { width: W - 120 });
    ...
    doc.fillColor(NAVY).fontSize(11).font("Helvetica-Bold")
      .text("CONDITIONS GÉNÉRALES", 40, y);
    y += 16;
    ...
    clauses.forEach((clause) => {
      doc.fillColor("#374151").fontSize(8.5).font("Helvetica")
        .text(clause, 40, y, { width: W - 80, align: "justify" });
      y += 24;
    });
    ...
    doc.fillColor(MID_GRAY).fontSize(8).font("Helvetica")
      .text("Signature du propriétaire", 50, y + 6);
    doc.fillColor(NAVY).fontSize(9).font("Helvetica-Bold")
      .text(`${owner.prenom} ${owner.nom}`, 50, y + 44);

    // Locataire
    doc.rect(W - 260, y, 220, 60).stroke("#D1D5DB");
    doc.fillColor(MID_GRAY).fontSize(8).font("Helvetica")
      .text("Signature du locataire", W - 250, y + 6);
    doc.fillColor(NAVY).fontSize(9).font("Helvetica-Bold")
      .text(`${renter.prenom} ${renter.nom}`, W - 250, y + 44);
    ...
    doc.fillColor("white").opacity(0.6).fontSize(7.5).font("Helvetica")
      .text(
        `SailingLoc — Agence Pandawan © ${new Date().getFullYear()} — Ce contrat est généré automatiquement après confirmation du paiement — sailingloc.fr`,
        0, H - 22, { width: W, align: "center" }
      );
    doc.opacity(1);

    // Numéro de page
    doc.fillColor("white").opacity(0.5).fontSize(7)
      .text(`Contrat n°${booking.id}`, W - 100, H - 22);
