    ...
    >
      <div className="relative w-full max-w-2xl rounded-2xl bg-white shadow-xl">
        ...
        <div className="p-6">
          ...
          <h2 className="mt-3 font-heading text-xl font-semibold leading-snug text-navy">
            {article.titre}
          </h2>
          <div className="mt-5 space-y-3 text-sm leading-relaxed text-gray-600 whitespace-pre-line">
            {article.contenu}
          </div>
          <div className="mt-6 flex gap-3">
            <Link
              to={article.lienBoats || "/boats"}
              onClick={onClose}
...
// SEO meta tags
  ...
  const openModal = (article, idx) => {
    setOpenArticle(article);
    setOpenGradient(GRADIENTS[idx % GRADIENTS.length]);
  };
    ...
    <div className="bg-cloud">
      ...
      <section className="bg-navy py-16 text-white">
        <div className="mx-auto max-w-4xl px-6 text-center">
          <Reveal>
            <span className="rounded-full border border-white/20 px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-white/70">
              Inspiration
            </span>
            <h1 className="mt-4 font-heading text-4xl font-semibold leading-tight sm:text-5xl">
              Actualités & guides<br />de la mer.
            </h1>
            <p className="mt-4 text-lg text-white/70">
              Actualités nautiques, guides de destinations, conseils pratiques —
              tout ce dont tu as besoin pour préparer ta prochaine aventure.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="bg-white py-8">
        <div className="mx-auto max-w-5xl px-6">
          <Reveal>
            <p className="text-xs font-semibold uppercase tracking-widest text-gray-400">Destinations tendance</p>
          </Reveal>
          <Reveal delay={80} className="mt-3 flex flex-wrap gap-2">
            {DESTINATIONS_TENDANCE.map((d) => (
              <Link
                key={d.label}
                to={`/boats?localisation=${encodeURIComponent(d.query)}`}
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-1.5 text-xs font-semibold transition ${
                  activeCategory === cat ? "bg-navy text-white" : "bg-cloud text-gray-600 hover:bg-gray-200"
                }`}
              >
                {cat}
              </button>
              ...
              <Reveal key={article.id} delay={i * 60}>
                ...
                >
                  <div className={`h-28 bg-gradient-to-br ${GRADIENTS[i % GRADIENTS.length]} flex items-end p-4`}>
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${CATEGORY_COLORS[article.categorie] || "bg-white/20 text-white"}`}>
                      {article.categorie}
                    </span>
                  </div>
                  <div className="flex flex-1 flex-col p-4">
                    ...
                    <h3 className="mt-1 font-heading text-sm font-semibold leading-snug text-navy">
                      {article.titre}
                    </h3>
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      ...
                      <span className="text-xs font-semibold text-sky opacity-0 transition group-hover:opacity-100">
                        Lire la suite →
                      </span>
                    </div>
                  </div>
      ...
      <section className="bg-navy py-14 text-white">
        <Reveal>
          <div className="mx-auto max-w-2xl px-6 text-center">
            ...
            <Link
              to="/boats"
              className="mt-6 inline-block rounded-full bg-white px-8 py-3 text-sm font-semibold text-navy transition hover:bg-cloud"
            >
