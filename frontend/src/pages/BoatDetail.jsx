...
function BookingTrustStrip() {
  const items = [
    ...
    {
      text: "Annulation gratuite jusqu'à 48h avant",
      icon: (
        <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <rect x="4" y="5" width="16" height="15" rx="2" />
          <path d="M4 10h16M8 3v4M16 3v4" />
          <path d="M9 15.5l2 2 4-4.2" />
        </svg>
      ),
    },
  ];
  return (
    <ul className="mt-4 space-y-2 border-t border-gray-100 pt-4">
      {items.map((item) => (
        <li key={item.text} className="flex items-center gap-2 text-xs text-gray-500">
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      <div className="mt-4 grid gap-10 lg:grid-cols-[1fr_340px]">
        {/* INFO — colonne gauche */}
        <div>
          <div className="relative h-80 overflow-hidden rounded-2xl bg-gradient-to-br from-navy to-sky shadow-[0_8px_30px_-10px_rgba(10,42,67,0.3)] sm:h-[26rem]">
            {activeImage ? (
            ...
            {boat.avecSkipper && (
              <span className="absolute right-4 top-4 inline-flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1.5 text-xs font-semibold text-navy shadow-sm backdrop-blur-sm">
                ⚓ Avec skipper
              </span>
            )}
          </div>
          ...
          {boat.User && (
            <div className="mt-6 flex items-center gap-3 rounded-xl border border-gray-100 bg-cloud/60 p-4">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-navy text-sm font-semibold text-sable">
                {boat.User.prenom?.charAt(0)}
              </div>
              <div className="text-sm text-gray-600">
                ...
                <span className="ml-2 inline-flex items-center gap-1 text-xs font-medium text-sky">
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-[0_2px_20px_rgba(10,42,67,0.08)]">
